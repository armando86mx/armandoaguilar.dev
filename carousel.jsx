...
(function () {
  ...
  function Carousel({ images, badge, tag, name }) {
    ...
    const onDown = (e) => { drag.current = { x: e.clientX, active: true, moved: 0 }; };
    const onMove = (e) => { if (drag.current.active) drag.current.moved = e.clientX - drag.current.x; };
    const onUp = () => {
      if (!drag.current.active) return;
      const m = drag.current.moved; drag.current.active = false;
      if (m < -45) go(1); else if (m > 45) go(-1);
    };

    return (
      <div className="aa-car">
        <div className="aa-car-stage"
          ...
          {badge && <span className="aa-car-badge">{badge}</span>}
          {tag && <span className="aa-car-tag">{tag}</span>}
          <button className="aa-car-btn prev" onClick={() => go(-1)} aria-label="Anterior">‹</button>
          <button className="aa-car-btn next" onClick={() => go(1)} aria-label="Siguiente">›</button>
        </div>
        <div className="aa-car-foot">
          <div className="aa-car-dots">
            {images.map((_, k) => (
              <button key={k} className={"aa-car-dot" + (k === i ? " on" : "")} onClick={() => setI(k)} aria-label={`Ir a ${k + 1}`} />
            ))}
          </div>
          <span className="aa-car-count">{String(i + 1).padStart(2, "0")} / {String(n).padStart(2, "0")}</span>
        </div>
      </div>
    );
  }
