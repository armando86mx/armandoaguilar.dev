...
function useReveal() {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver((es) => es.forEach((e) => { if (e.isIntersecting) { setShown(true); io.disconnect(); } }), { threshold: 0.14 });
...
function Reveal({ children, className = "", style }) {
  ...
  return <div ref={ref} className={`reveal ${shown ? "in" : ""} ${className}`} style={style}>{children}</div>;
}
...
function Feature({ p, i, t }) {
  const flip = i % 2 === 1;
  return (
    <Reveal>
      <article className={`bp-feat ${flip ? "flip" : ""}`}>
        <div className="bp-feat-text">
          <div className="bp-feat-area">{t(p.area)}</div>
          <h3 className="bp-feat-title">{t(p.title)}</h3>
          <div className="bp-feat-name">{t(p.name)} · {t(p.kind)}</div>
          <div className="bp-feat-body">
            <p className="prob"><span className="lbl">{t(C.ui.problem)}</span>{t(p.problem)}</p>
            <p className="res"><span className="lbl">{t(C.ui.result)}</span>{t(p.result)}</p>
          </div>
          <div className="bp-feat-tags">{p.tags.map((tg, k) => <span key={k}>{t(tg)}</span>)}</div>
          {p.ai && <div className="bp-feat-ai">{t(C.ui.builtAi)}</div>}
          {p.linkPending && (
            <div className="bp-pbtns">
              {p.link
                ? <a className="bp-pbtn primary" href={p.link} target="_blank" rel="noopener">{t(C.ui.viewSite)} ↗</a>
                : <span className="bp-pbtn soon">{t(C.ui.viewSite)} <span className="s">· {t(C.ui.soon)}</span></span>}
            </div>
          )}
        </div>
...
function App() {
  ...
  useEffect(() => { document.documentElement.setAttribute("data-theme", theme); document.documentElement.lang = lang; }, [theme, lang]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    C.nav.forEach((n) => { const el = document.getElementById(n.id); if (el) io.observe(el); });
  ...
  return (
    <div className="bp">
      ...
      <nav className={`bp-nav ${scrolled ? "scrolled" : ""}`}>
        <div className="bp-nav-in">
          <a href="#inicio" className="bp-mark"><span className="pin" />A.AGUILAR<span className="bp-mark-suf"><b>·</b>{lang==="es"?"INFRA/NUBE":"INFRA/CLOUD"}</span></a>
          ...
          <div className="bp-ctrls">
            ...
            {/* ícono por CSS (no por estado) → idéntico en SSR y cliente, sin mismatch de hidratación */}
            <button className="bp-tg bp-tg-theme" onClick={()=>setTheme(theme==="dark"?"light":"dark")} aria-label={lang==="es"?"Cambiar tema":"Toggle theme"}><span className="ic-moon">☾</span><span className="ic-sun">☀</span></button>
            <button className="bp-burger" onClick={()=>setMenuOpen((o)=>!o)} aria-label={lang==="es"?"Menú":"Menu"} aria-expanded={menuOpen}>{menuOpen?"✕":"☰"}</button>
          </div>
        </div>
      </nav>
      <div className={`bp-mobnav ${menuOpen?"open":""}`}>
        {C.nav.map((n)=><a key={n.id} href={`#${n.id}`} className={active===n.id?"active":""} onClick={()=>setMenuOpen(false)}>{t(n)}</a>)}
      </div>

      <main className="bp-main">
        {/* HERO */}
        <header id="inicio" className="bp-shell bp-hero">
          <div className="bp-kicker"><i className="bp-dot" />{t(C.hero.kicker)}</div>
          <div className="bp-hero-grid">
            <div>
              <h1 className="bp-headline">
                <span className="br">{head[0]}</span>
                <span className="br"><span className="em">{head[1]}</span></span>
                <span className="br">{head[2]}</span>
              </h1>
              <p className="bp-sub">{t(C.hero.sub)}</p>
              <p className="bp-role">// {t(C.hero.role)}</p>
              <div className="bp-cta">
                <a href="#proyectos" className="bp-btn">{t(C.hero.ctaPrimary)} →</a>
                <a href="#contacto" className="bp-btn ghost">{t(C.hero.ctaSecondary)}</a>
              </div>
            </div>
            <div className="bp-hero-right">
              ...
              <div className="bp-social">
                <a className="bp-soc gh" href={C.contact.github} target="_blank" rel="noopener">GitHub <span>↗</span></a>
                <a className="bp-soc li" href={C.contact.linkedin} target="_blank" rel="noopener">LinkedIn <span>↗</span></a>
              </div>
            </div>
          </div>
        </header>
        ...
        <section id="mente" className="bp-shell bp-section">
          ...
          <Reveal>
            <p className="bp-analytic-lead">{(() => {
              ...
              const txt = t(C.analytic.intro); const i = txt.indexOf(key);
              if (i < 0) return txt;
              return (<React.Fragment>{txt.slice(0,i)}<span className="hl">{txt.slice(i)}</span></React.Fragment>);
            })()}</p>
          </Reveal>
          <Reveal>
        ...
        <section id="habilidades" className="bp-shell bp-section">
          ...
          <Reveal>
            <div className="bp-cert-band">
              ...
              <div className="bp-cert-band-grid">
                {C.certs.items.map((c,i)=>(
                  <div className="bp-cert-feat" key={i}>
                    <div className="nm"><i/>{c.name}</div>
                    <div className="is">{c.issuer} · {t(c.note)}</div>
                    {c.link && <a className="bp-cert-verify" href={c.link} target="_blank" rel="noopener">{t(C.ui.verify)} ↗</a>}
                  </div>
                ))}
              </div>
            </div>
            {C.certs.note && <p className="bp-certs-note">{t(C.certs.note)}</p>}
          </Reveal>
        </section>
        ...
        <footer className="bp-shell bp-footer">
          <span>© {new Date().getFullYear()} {C.meta.name} · {t(C.meta.role)}</span>
